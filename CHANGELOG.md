...
The Codex CLI provider retains the existing protocol and preservation contract.
...
The Codex CLI provider retains prior setup identity and written ownership.
...
declares.** The rule a model reads while working on Codex CLI said the product
...
**A walk written when a surface held one kind.** Codex CLI's table gained a
...
**Codex CLI's table gained a kind.** Where a product loads a plugin from a
...
**Codex CLI declares the `instruction` kind**, and the surface that routes it is
...
Codex CLI's provider publishes a projection profile per *scope* -- a second
...
Starts Codex CLI, and takes over a target the estate before this one
still claims.
...
Installs Codex CLI itself, not only its configuration.
...
Codex CLI harness configuration in a caller-named target directory.
