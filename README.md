...
instructions, agents, commands, hooks and settings together, in one step.
...
```bash
CODEX_INSTALL_DIR=/opt/codex-setup-system sh install.sh
```

The same variable on both scripts, and it is `CODEX_INSTALL_DIR`
...
```bash
ai-stp provider conformance --harness codex \
  --executable ~/.local/bin/codex-setup-system \
...
docker run --rm -v "$HOME/.config:/config" \
  ghcr.io/nddev-opennetwork/codex-setup-system:0.0.56 \
